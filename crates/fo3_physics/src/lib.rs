...
pub use rapier::{
    collision_shape_to_rapier, rigid_body_data_to_rapier, RapierCharacterController,
    RapierPhysicsWorld,
};
...
mod tests {
    ...
    fn test_box_collider_conversion_and_raycast() {
        ...
        // 床の上面は Z = 5.0
        assert!(
            (hit.point.z - 5.0).abs() < 0.01,
            "ヒット位置が床の上面 Z=5.0 と一致しません: {:?}",
            hit.point
        );
        assert_eq!(
            hit.normal,
            Vec3::new(0.0, 0.0, 1.0),
            "床の法線が上向き (0, 0, 1) ではありません"
        );
    }
    ...
    fn test_character_controller_grounding_and_gravity() {
        ...
        assert!(
            state.is_grounded,
            "床に到達したのに接地フラグが立っていません"
        );
        // 底面が床 Z = 0.0 に接するため、中心 Z は 64.0 付近
        assert!(
            (controller.position.z - 64.0).abs() < 1.0,
            "キャラクタの接地中心高度が不正です: {}",
    ...
    fn test_interaction_raycast_and_occlusion() {
        ...
        let wall_data = NifCollisionData {
            bodies: vec![wall_body],
        };
        world.add_nif_collision_with_user_data(
            &wall_data,
        ...
        let item_data = NifCollisionData {
            bodies: vec![item_body],
        };
        world.add_nif_collision_with_user_data(
            &item_data,
        ...
        assert_eq!(
            hit_res.user_data, 0,
            "手前の壁で遮蔽されず奥のアイテムにヒットしてしまいました"
        );
        assert!(
            (hit_res.point.x - 45.0).abs() < 1e-3,
            "壁の手前表面にヒットしていません"
        );
        ...
        assert_eq!(
            hit_y.unwrap().user_data,
            0x00054321,
            "遮蔽物のないアイテムの FormID が取得できませんでした"
        );
