...
fn test_new_game_cg00_progression_simulation() {
    ...
    let quest_map = reader
        .read_all_quests_map()
        .expect("Failed to read quest map");
    let script_map = reader
        .read_all_scripts_map()
        .expect("Failed to read script map");
    let pack_map = reader
        .read_all_packages_map()
        .expect("Failed to read pack map");
    let (dialogue_map, info_map) = reader
        .read_all_dialogues_map()
        .expect("Failed to read dialogues");
    ...
    for (_, q) in quest_map.clone() {
        vm.edid_map
            .insert(q.editor_id.to_ascii_uppercase(), q.form_id);
    ...
    assert_eq!(
        stage, 5,
        "Stage 0 のスクリプト実行により Stage 5 に自動連鎖遷移すること"
    );

    // テレポート要求 (player.moveto CG00PlayerStartMarker) が発行されていること
    assert!(
        vm.teleport_requests
            .iter()
            .any(|(_, marker)| marker == "CG00PlayerStartMarker"),
    ...
    assert_eq!(
        stage_after_timer3, 10,
        "Stage 9 満了後に自動的に Stage 10 (父親の台詞開始) へ進むこと"
    );
    ...
    assert!(
        vm.sound_queue
            .iter()
            .any(|s| s.eq_ignore_ascii_case("QSTBirthStart")),
        "Stage 6 で QSTBirthStart サウンドが要求されていること"
    );
    assert!(
        vm.sound_queue
            .iter()
            .any(|s| s.eq_ignore_ascii_case("QSTBabyCry")),
    ...
    let sub = sound_engine
        ...
        .unwrap();
    println!(
        "表示中の台詞字幕: FormID=0x{:08X}, Speaker={}",
        sub.form_id.0, sub.speaker
    );
    ...
    assert_eq!(
        stage_after_speech, 18,
        "父親の台詞完了により Stage 18 へ自動遷移すること"
    );
    ...
    assert!(
        (!sound_engine.active_subtitles.is_empty()),
        "父親終了後に母親の台詞が自律開始されること"
    );
    let mom_sub = sound_engine
        .active_subtitles
        .values()
        .next()
        .map(|(s, _)| s)
    ...
    assert_eq!(
        stage_name_prompt, 38,
        "名前入力準備 (Stage 38) に進行すること"
    );
    ...
    assert_eq!(
        stage_name, 40,
        "タイマー満了で Stage 40 (GetPlayerName) へ進むこと"
    );
    ...
    assert!(
        chargen_menu.is_active(),
        "GetPlayerName により ChargenMenu がアクティブになること"
    );
