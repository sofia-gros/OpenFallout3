    ...
    let (
        ...
        | ViewerTarget::NewGame { .. } => {
                ...
                for refr in refrs {
                    ...
                    if let Some(npc) = npc_map.get(&refr.base_object) {
                        let pos =
                            glam::Vec3::new(refr.position[0], refr.position[1], refr.position[2]);
                        let rot =
                            glam::Vec3::new(refr.rotation[0], refr.rotation[1], refr.rotation[2]);
            ...
            let mut animated_statics: Vec<(
                ...
                fo3_gamebryo_core::NiTransform,
                Option<std::sync::Arc<fo3_render::animation::AnimationClip>>,
                String,
            )> = Vec::new();
            ...
            let cell_inputs: Vec<(
                ...
                .map(|((cell, _, land), items)| {
                    ...
                    for (form_id, n, t, record_type, model_path) in items {
                        if let Some(clip) =
                            fo3_render::animation::AnimationClip::from_transform_controllers(
                                n.as_ref(),
                            )
                        {
                        ...
                        } else if *record_type == fo3_esm::types::REC_ACTI
                            || *record_type == fo3_esm::types::REC_MSTT
                        {
                            animated_statics.push((
                                *form_id,
                                n.clone(),
            ...
            if !cell_npcs.is_empty() {
                ...
                for npc in &cell_npcs {
                    ...
                    let head_diffuse_override =
                        if let Some(ref fgts) = npc.facegen_texture_symmetric {
                        ...
                        if let Some(cached) = default_idle_arcs.get(&base_id) {
                            ...
                            .and_then(|pkg| {
                                ...
                                if kf_paths.is_empty() {
                                    kf_paths.push(
                                        "meshes\\characters\\_male\\idleanims\\mtidle.kf"
                                            .to_string(),
                                    );
                                }
                                for path in &kf_paths {
                                    if let Ok(bytes) = vfs.read(path) {
                                        ...
                                        if let Ok(kf) = NifFile::read(&mut cursor) {
                                            if let Some(clip) =
                                                fo3_render::AnimationClip::from_kf(&kf)
                                            {
