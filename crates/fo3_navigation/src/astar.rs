    ...
    }) = heap.pop()
    {
        ...
        for (edge_idx, &edge_neighbor) in curr_node.triangle.edges.iter().enumerate() {
            ...
            if edge_neighbor >= 0 {
                ...
                // 外部接続エッジの解決 (NVEX)
                if let Some(&(target_mesh, target_tri)) =
                    graph
                        .external_links
                        .get(&(navmesh_id, triangle_idx, edge_idx))
                {
...
mod tests {
    use super::*;
    use crate::graph::NavGraph;
    use fo3_esm::records::navm::{NavMeshExternalConnection, NavMeshRecord, NavMeshTriangle};
    use fo3_esm::types::FormId;
    use glam::Vec3;
    ...
    fn test_external_connection_astar() {
        ...
        let mut navm1 = NavMeshRecord {
            ...
            external_connections: vec![NavMeshExternalConnection {
        ...
        let mut navm2 = NavMeshRecord {
            form_id: FormId(2),
            vertices: vec![[10.0, 0.0, 0.0], [20.0, 0.0, 0.0], [10.0, 10.0, 0.0]],
            triangles: vec![NavMeshTriangle {
                vertices: [0, 1, 2],
                edges: [-1, -1, -1],
