...
| :--- | :--- | :--- |
| **2026-09-13** | **Phase 10: Event-Driven Scripting & Quest Progression (Event-Driven Scripting & Quest Engine)**<br>・**Object Script & Event Block Binding**: Resolving `SCRI` subrecords across `REFR`, `DOOR`, `CONT`, `ACTI`; dispatching `Begin OnActivate` / `OnAdd` / `OnEquip` / `GameMode` blocks<br>・**Event Dispatcher & Activation Suppression**: Prioritized `OnActivate` triggers on E-key interaction with deferred opening via in-script `Activate()`<br>・**VM Opcode & Control Flow Extension**: Parsing `If` / `ElseIf` / `Else` / `EndIf`, variable assignment, object reference-qualified calls (`Ref.Command`), `SetStage` / `GetStage`<br>・**Authentic Quest Vertical Slice**: Megaton "Moriarty's Saloon" quest progression (dialogue with Colin Moriarty, terminal access, cabinet picking linked to stage updates) | `fo3_esm`<br>`fo3_script`<br>`fo3_viewer` |
| **2026-09-13** | **Phase 9: Script VM & Dialogue 2D UI Renderer (Script VM & Dialogue UI)**<br>・**Font & 2D UI Renderer (`crates/fo3_render/ui`)**: Authentic `.fnt` (OpenMW `fontloader.cpp` standard) parser, fallback embedded bitmap font, frontmost 2D overlay render pipeline<br>・**ESM Script, Dialogue, & Terminal Parser (`crates/fo3_esm`)**: `SCPT` (bytecode/source/local variables), `GLOB`, `DIAL` / `INFO` (24-byte `CTDA` condition expressions / Result Script), `TERM` (menu items, body text, Result Script)<br>・**Script VM Engine (`crates/fo3_script`)**: Condition evaluation engine, local/global variable storage, quest stage progression, inventory checks/grants, unlock states, Result Script execution<br>・**Dialogue & Terminal In-Game UI (`crates/fo3_viewer/ui`)**: `dialog_menu.xml` / `terminal_menu.xml` compliant layout, real-time font rendering, keyboard selection with immediate Result Script triggers | `fo3_esm`<br>`fo3_script`<br>`fo3_render`<br>`fo3_viewer` |
...
│   ├── fo3_script/         # Script VM, condition evaluation engine (CTDA), variables
│   ├── fo3_render/         # wgpu rendering pipeline, scene graph, skinning, animation, 2D UI
...
| Key / Mouse | Action |
| :--- | :--- |
| **Tab / M** | Toggle camera mode (Orbital orbit ⇔ Walkthrough mode) |
| **V** | Toggle First-Person ⇔ Third-Person (Over-the-shoulder chase camera) |
...
| **Mouse Wheel** | Zoom in / out (Third-person camera zoom & seamless first-person transition) |
...
| **E** | Interact (Door transition, door/container open/close, item pickup, NPC talk, terminal access) |
| **T** | Exit dialogue / Back or logout in terminal menu |
| **Esc** | Exit viewer |
...
| **Phase 8: Character & Camera** | Player actor assembly, seamless 1st arms ⇔ 3rd person body culling, GMST camera (eye height 124, over-shoulder X 30, raycast anti-clipping), KF locomotion state machine (Walk/Run/Jump) | **Complete** |
| **Phase 9: Script VM & Dialogue** | DIAL/INFO dialogue topics, TERM terminal UI, font & 2D UI renderer, CTDA condition evaluation, Result Script VM execution | **Complete** |
| **Phase 10: Event-Driven Scripting & Quests** | Object script binding, event block dispatch (`OnActivate`), control flow (`If/Else`), QUST stage engine, Megaton authentic vertical slice | **In Progress (Planning Complete)** |
